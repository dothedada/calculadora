    ...
    operar: (op) => {
        ...
        if (parseFloat(+respuesta).toString().length > 10) {
            if (Math.round(+respuesta).toString().length > 10) {
                ...
                respuesta = +parseFloat(+respuesta).toFixed(
                    10 - Math.round(+respuesta).toString().length
                );
            }
        }
...
const memoria = {
    ...
    valor: () => {
        if (
            !pantalla.celdaActiva() ||
            operador.classList.contains('operacion--lock')
        ) {
    ...
    crearEspacioMemoria: (boton) => {
        ...
        memValor.textContent =
            boton === '=' ? resultado.textContent : memoria.valor();
        ...
        if (boton === '=') {
            ...
            if (resultado.textContent.length + memOp.textContent.length > 24) {
                const deMas =
                    resultado.textContent.length +
                    memOp.textContent.length -
                    24;
            ...
            if (!dispMemoria.children[0]) {
                ...
                dispMemoria.insertBefore(
                    memDiv,
                    dispMemoria.children[memoria.casillas[0]]
                );
            }
        } else {
        ...
        memDiv.addEventListener('click', () => {
            ...
            pantalla.celdaActiva().textContent =
                memDiv.lastElementChild.textContent;
        });
    },
};
...
const ingresarDigito = (boton) => {
    ...
    if (boton === '.') {
        pantalla.celdaActiva().textContent = `${+pantalla
            .celdaActiva()
            .textContent.split('.')
            .join('')}.`;
    }
    ...
    if (/^[0-9]$/.test(boton)) {
        if (
            pantalla.celdaActiva().textContent === '0' ||
            pantalla.celdaActiva().textContent === '3rr0r'
        ) {
...
const ingresarOperacion = (op) => {
    memoria.vaBorrarCasilla = false;
    if (operando1.textContent === '' || (op === '=' && !operando2.textContent))
        return;
...
const manejarMemoria = (boton) => {
    ...
    if (/^M/.test(boton.textContent)) {
        const espacioDisponible = Array.from(dispMemoria.children).find(
            (casilla) => {
                return (
                    casilla.firstElementChild.textContent ===
                    `${boton.textContent}»`
                );
            }
        );
        if (memoria.vaBorrarCasilla) {
            ...
            memoria.vaBorrarCasilla = false;
            memoria.casillas[0]--;
            if (!memoria.casillas[0])
                teclado.firstElementChild.lastElementChild.classList.add(
                    'boton--mVacia'
                );
            return;
        }
        if (!espacioDisponible) {
            ...
            teclado.firstElementChild.lastElementChild.classList.remove(
                'boton--mVacia'
            );
        } else {
            if (!pantalla.celdaActiva()) pantalla.reset();
            pantalla.celdaActiva().textContent = '';
            pantalla.celdaActiva().textContent =
                espacioDisponible.lastElementChild.textContent;
        }
        return;
    }
...
const tecladoFisico = (teclaEvento, tecladoDisp) => {
    ...
    if (tecladoDisp === 'operacion')
        tecladoMapeo = teclado.children[2].children;
...
teclado.addEventListener('click', (event) => {
    ...
    if (/^[0-9.]|^\+.\-$/.test(boton.textContent))
        ingresarDigito(boton.textContent);
    if (/^[^0-9A-M.]{1,2}$/.test(boton.textContent))
        ingresarOperacion(boton.textContent);
});
